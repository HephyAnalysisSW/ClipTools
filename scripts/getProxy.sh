...
# renew proxy within singularity
if [ -d "/.singularity.d/" ]; then
    python -c 'import os; from proxyHelper import renew_proxy; renew_proxy(filename=os.getenv("X509_USER_PROXY"),request_time=192,min_time=os.getenv("min_proxyTime"))'
    exit
fi
...
# renew proxy outside of singularity
